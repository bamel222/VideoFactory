...
export default function MonetizationPage() {
  ...
  const load = async () => {
    try {
      setPriorities(await api("/monetization/priorities"));
    } catch (e) {
  ...
  return (
    <div>
      <PageHeader title="Monétisation & Optimisation" subtitle="Scoring business des sujets et priorisation des séries par valeur/coût." />
      ...
      <div className="card mb">
        ...
        <form onSubmit={compute}>
          <div className="grid" style={{ gridTemplateColumns: "1fr 140px 150px 120px 130px auto", alignItems: "end" }}>
            <Field label="Sujet"><input value={form.topic} onChange={(e) => setForm({ ...form, topic: e.target.value })} required /></Field>
            <Field label="Type">
              <select value={form.kind} onChange={(e) => setForm({ ...form, kind: e.target.value })}>
                <option value="documentary">Documentaire</option><option value="cartoon">Cartoon</option>
              </select>
            </Field>
            <Field label="Langues (csv)"><input value={form.languages} onChange={(e) => setForm({ ...form, languages: e.target.value })} /></Field>
            <Field label="Durée min"><input type="number" step="0.5" value={form.duration_min} onChange={(e) => setForm({ ...form, duration_min: Number(e.target.value) })} /></Field>
            <Field label="Risque droits (0-1)"><input type="number" step="0.05" min="0" max="1" value={form.risk} onChange={(e) => setForm({ ...form, risk: Number(e.target.value) })} /></Field>
        ...
        <div className="table-wrap">
          <table className="table">
            ...
            <tbody>
              {priorities.map((p) => (
                <tr key={p.series_id}>
                  <td className="muted">{p.series_id}</td>
                  <td>{p.title}</td>
                  <td><span className="badge gray">{p.kind}</span></td>
